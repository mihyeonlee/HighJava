...
/*
    ...
    해시함수에 의해 얻어지는 값은 해시값, 해시코드, 해시체크섬 또는 간단하게 해시라고도 한다. 
    
  HashSet, HashMap, HashTable 과 같은 객체들을 사용할 경우 객체가 서로 같은지를 비교하기 위해 equals()메서드와 hashCode()메서드를 호출한다. 
     그래서 객체가 서로 같은지 여부를 결정하려면 두 메서드를 재정의 해야 한다. 두개 다 짝꿍같은 애들임 
  ...
  (equals()로 모두 비교하기에는 부담이 크다. 때문에 hashCode()로 검사한뒤 같은 객체이므로 그때 equals로 비교해준다.)
  
  
  ** equals()메서드와 hashCode()메서드에 관련된 규칙
    1. 두 객체가 같은면 반드시 같은 hashCode를 가져야한다.
    ...
    - hashCode()메서드에서 사용하는 '해싱알고리즘'에서 서로 다른 객체에 대하여 같은 
      hashCode값을 만들어 낼 수 있다. 그래서 객체가 같지 않더라도 hashCode가 같을 수 있다.(이럴 땐 equals로 확인한다.) 
 */
...
class Person{
	...
	public boolean equals(Object obj) {
		if (this == obj) //객체가 같아야한다. p1.equals(p2)
			return true; 
		if (obj == null)
			return false;
		if (getClass() != obj.getClass())  // 만든 클래스가 같은지 비교 
			return false;
