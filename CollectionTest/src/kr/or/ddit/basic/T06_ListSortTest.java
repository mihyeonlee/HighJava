...
/*
  정렬과 관련된 interface는 Comparable과 Comparator 이렇게 두가지가 있다.
  	...
  	- 보통 객체 자체에 정렬기준을 넣기 위해서는 Comparable을 구현하고 정렬 기준을 별도로 구현하고 싶을 때는 Comparator를 구현하여 사용하면 된다. 
...
// 정렬방식을 결정하는 class는 Comparator라는 인처페이스를 구현해야한다.
...
class Desc implements Comparator<String>{
	/*
	  ...
	  - String 객체에는 정렬을 위해서 compareTo()메서드가 구현되어 있는데 이 메서드의 반환값은 오름차순에 맞게 반환되도록 구현되어 잇다. 
	  (Wrapper클래스와 Date, File클래스에도 구현되어 있다.)
	 */
	@Override
	public int compare(String str1, String str2) {
		
		return str1.compareTo(str2) * -1; //String은 이미 정렬하는객체가 있기때문에 그걸 사용한것일 뿐임 
	}
