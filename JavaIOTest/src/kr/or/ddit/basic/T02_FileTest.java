...
public class T02_FileTest {
	public static void main(String[] args) {
		...
		if(f1.exists()) {
			...
			File f4 = new File("d:/DB");
			
			displayFileList(f4);
	}
	
	//지정된 디렉토리(폴더)에 포함된 디렉토리 목록을 보여주는 메서드
	private static void displayFileList(File dir) {
		System.out.println("["+dir.getAbsolutePath()+"] 디렉토리의 내용");
		// 디렉토리 안의 모든 파일 목록을 가져온다. 메서드 배열타입의 파일리스트반환
		File[] files = dir.listFiles();
		
		//하위 디렉토리 정보를 저장할 ArrayList생성(File배열의 인덱스 저장)
		...
		for(int i =0;i<files.length;i++) {
			...
			if(files[i].isDirectory()) {
				...
				size = files[i].length()+"";
				attr = files[i].canRead() ? "R" : " "; //읽기가능한지 true,false 
				attr += files[i].canWrite() ? "W" : " ";
				attr += files[i].isHidden() ? "H" : " "; //숨김파일인지
			}
			//printf format문자열 추출할때 %s(String) %5s(최대5자String올 공간) 
